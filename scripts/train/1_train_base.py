...
def load_resume_checkpoint(
    ...
    """Load model/optimizer/scheduler state for resume."""
    checkpoint = torch.load(checkpoint_path, map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])
    if weights_only:
        print(f"Loaded model weights from: {checkpoint_path} (fresh optimizer/scheduler)")
        return 0, 0.0
    ...
    if best_val_acc <= 0.0 and log_path and os.path.exists(log_path):
        inferred_best = 0.0
        with open(log_path, "r") as f:
            for raw_line in f:
                ...
                record_val_acc = record.get("val_acc")
                if isinstance(record_epoch, int) and record_epoch < start_epoch and isinstance(record_val_acc, (int, float)):
                    inferred_best = max(inferred_best, float(record_val_acc))
        best_val_acc = inferred_best
    print(
...
def maybe_trim_log_for_resume(log_path: str, start_epoch: int):
    """Remove stale JSONL records at or beyond the resume epoch."""
    ...
    with open(log_path, "r") as f:
        for raw_line in f:
            ...
            try:
                ...
                continue
            record_epoch = record.get("epoch")
            if isinstance(record_epoch, int) and record_epoch >= start_epoch:
                trimmed += 1
                continue
            kept_lines.append(raw_line)
    ...
    if trimmed > 0:
        ...
        print(f"Trimmed {trimmed} stale log entries from: {log_path}")
...
def main():
    ...
    parser.add_argument(
        "--auto-resume",
        action="store_true",
        help="Resume from checkpoints/base/<suite>_seed_<seed>_last.pt or _best.pt if available",
    )
    parser.add_argument(
        "--resume-weights-only",
        action="store_true",
        help="Load only model weights from --resume and start a fresh optimizer/scheduler/log",
    )
    ...
    explicit_worker_cfg = ("num_workers" in training_config) or (env_workers is not None)
    if os.name == "nt" and use_adv_train and not explicit_worker_cfg:
        # Windows + adversarial training + large ImageNet-sized tensors is prone to
        # random worker/shared-storage failures. Default to the safer single-process
        # loader unless the user explicitly overrides it.
        num_workers = 0
        pin_memory = False
        persistent_workers = False
        print("Windows adversarial training detected: defaulting to num_workers=0, pin_memory=False")
    ...
    best_save_path = f"checkpoints/base/{args.suite}_seed_{args.seed}_best.pt"
    last_save_path = f"checkpoints/base/{args.suite}_seed_{args.seed}_last.pt"
    ...
    if resume_path is None and args.auto_resume:
        ...
        elif os.path.exists(best_save_path):
            resume_path = best_save_path
    ...
    if resume_path is not None:
        ...
        if args.resume_weights_only:
            if os.path.exists(log_path):
                print(f"Starting fresh log for weights-only resume: {log_path}")
                os.remove(log_path)
        else:
            maybe_trim_log_for_resume(log_path, start_epoch)
            if start_epoch >= max_epochs:
                print(
                    f"Checkpoint already reached epoch {start_epoch - 1}, "
                    f"which is >= configured max_epochs={max_epochs}. Nothing to do."
                )
                return
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        if val_metrics['val_acc'] > best_val_acc:
            ...
            save_checkpoint(base_model, optimizer, scheduler, epoch, best_save_path, best_val_acc)

        # Save rolling checkpoint for resume support
        save_checkpoint(base_model, optimizer, scheduler, epoch, last_save_path, best_val_acc)
    ...
    # Save final model
    save_checkpoint(base_model, optimizer, scheduler, max_epochs - 1, final_save_path, best_val_acc)
