param(
    ...
    [string]$Suite = "fourier_alias_orbit_salun_forget0_10shot",
...
if (-not $SkipTrain) {
    foreach ($seed in $Seeds) {
        Invoke-PythonStep -Name "train Fourier attack seed=$seed" -CommandArgs @(
...
if (-not $SkipTransfer) {
    $artifactDir = "checkpoints/fourier_attack/$Suite`_seed_$TransferSeed"
    $transferArgs = @(
        "scripts/14_transfer_universal_patch.py",
        "--config", $Config,
        "--patch-dir", $artifactDir,
    ...
    Invoke-PythonStep -Name "transfer Fourier attack" -CommandArgs $transferArgs
}

Write-Host ""
Write-Host "Fourier alias attack pipeline complete."
Write-Host "Key outputs:"
Write-Host "  checkpoints/fourier_attack/$Suite`_seed_<seed>"
