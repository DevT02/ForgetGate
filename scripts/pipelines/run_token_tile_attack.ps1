param(
    ...
    [string]$Suite = "tile_resonance_orbit_salun_forget0_10shot",
...
if (-not $SkipTrain) {
    foreach ($seed in $Seeds) {
        Invoke-PythonStep -Name "train token-tile attack seed=$seed" -CommandArgs @(
...
if (-not $SkipTransfer) {
    $artifactDir = "checkpoints/tile_attack/$Suite`_seed_$TransferSeed"
    $transferArgs = @(
    ...
    Invoke-PythonStep -Name "transfer token-tile attack" -CommandArgs $transferArgs
}

Write-Host ""
Write-Host "Token-tile attack pipeline complete."
Write-Host "Key outputs:"
Write-Host "  checkpoints/tile_attack/$Suite`_seed_<seed>"
