#!/usr/bin/env python3
"""
Run the paper pipeline (resume-safe).
"""
...
def main():
    parser = argparse.ArgumentParser(description="ForgetGate resume pipeline (skip completed)")
    ...
    for seed in seeds:
        ...
        if os.path.exists(base_ckpt):
            print(f"[SKIP] Base seed {seed}")
        else:
            run(["python", "scripts/1_train_base.py", "--config", config, "--suite", base_suite, "--seed", str(seed)])

        oracle_ckpt = f"checkpoints/oracle/{oracle_suite}_seed_{seed}_final.pt"
        if os.path.exists(oracle_ckpt):
            print(f"[SKIP] Oracle seed {seed}")
        else:
            run(["python", "scripts/1b_train_retrained_oracle.py", "--config", config, "--suite", oracle_suite, "--seed", str(seed)])

        if os.path.exists(f"results/logs/{unlearn_ce}_seed_{seed}_history.json"):
            print(f"[SKIP] Unlearn CE seed {seed}")
        else:
            run(["python", "scripts/2_train_unlearning_lora.py", "--config", config, "--suite", unlearn_ce, "--seed", str(seed)])

        if os.path.exists(f"results/logs/{unlearn_kl}_seed_{seed}_history.json"):
            print(f"[SKIP] Unlearn KL seed {seed}")
        else:
            run(["python", "scripts/2_train_unlearning_lora.py", "--config", config, "--suite", unlearn_kl, "--seed", str(seed)])

        if args.run_salun:
            if os.path.exists(f"results/logs/{unlearn_salun}_seed_{seed}_history.json"):
                print(f"[SKIP] Unlearn SalUn seed {seed}")
            else:
                run(["python", "scripts/2_train_unlearning_lora.py", "--config", config, "--suite", unlearn_salun, "--seed", str(seed)])

        if args.run_scrub:
            if os.path.exists(f"results/logs/{unlearn_scrub}_seed_{seed}_history.json"):
                print(f"[SKIP] Unlearn SCRUB seed {seed}")
            else:
    ...
    for seed in seeds:
        for k in kshots:
            oracle_log = f"results/logs/vpt_oracle_vit_cifar10_forget0_{k}shot_seed_{seed}.jsonl"
            if os.path.exists(oracle_log):
                print(f"[SKIP] VPT Oracle seed {seed} k={k}")
            else:
                run(["python", "scripts/3_train_vpt_resurrector.py", "--config", config,
                     "--suite", f"vpt_oracle_vit_cifar10_forget0_{k}shot", "--seed", str(seed)])

            kl_log = f"results/logs/vpt_resurrect_kl_forget0_{k}shot_seed_{seed}.jsonl"
            if os.path.exists(kl_log):
                print(f"[SKIP] VPT KL seed {seed} k={k}")
            else:
                run(["python", "scripts/3_train_vpt_resurrector.py", "--config", config,
    ...
    if args.run_fulldata_vpt:
        for seed in seeds:
            if os.path.exists(f"results/logs/{vpt_ce}_seed_{seed}.jsonl"):
                print(f"[SKIP] VPT CE seed {seed}")
            else:
                run(["python", "scripts/3_train_vpt_resurrector.py", "--config", config, "--suite", vpt_ce, "--seed", str(seed)])

            if os.path.exists(f"results/logs/{vpt_kl}_seed_{seed}.jsonl"):
                print(f"[SKIP] VPT KL seed {seed}")
            else:
                run(["python", "scripts/3_train_vpt_resurrector.py", "--config", config, "--suite", vpt_kl, "--seed", str(seed)])

            if args.run_salun:
                if os.path.exists(f"results/logs/{vpt_salun}_seed_{seed}.jsonl"):
                    print(f"[SKIP] VPT SalUn seed {seed}")
                else:
                    run(["python", "scripts/3_train_vpt_resurrector.py", "--config", config, "--suite", vpt_salun, "--seed", str(seed)])

            if args.run_scrub:
                if os.path.exists(f"results/logs/{vpt_scrub}_seed_{seed}.jsonl"):
                    print(f"[SKIP] VPT SCRUB seed {seed}")
                else:
                    run(["python", "scripts/3_train_vpt_resurrector.py", "--config", config, "--suite", vpt_scrub, "--seed", str(seed)])

    # 4) Evaluation
    for seed in seeds:
        eval_log = f"results/logs/{eval_baselines}_seed_{seed}_evaluation.json"
        if os.path.exists(eval_log):
            print(f"[SKIP] Eval baselines seed {seed}")
        else:
            run(["python", "scripts/4_adv_evaluate.py", "--config", config, "--suite", eval_baselines, "--seed", str(seed)])

    # 5) Analysis
    if os.path.exists("results/analysis/kshot_analysis.png"):
        print("[SKIP] k-shot analysis")
    else:
        run(["python", "scripts/analyze_kshot_experiments.py", "--seeds"] + [str(s) for s in seeds])

    if os.path.exists("results/analysis/comparison_table_markdown.txt"):
