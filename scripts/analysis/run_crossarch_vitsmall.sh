...
set -u
cd "$(dirname "$0")/../.."
CFG=configs/suites/crossarch_vit_small.yaml
for suite in unlearn_salun_vit_small_ca_forget0 unlearn_scrub_vit_small_ca_forget0 unlearn_ceascent_vit_small_ca_forget0 unlearn_uniformkl_vit_small_ca_forget0 unlearn_featurescrub_vit_small_ca_forget0 unlearn_smoothedmargin_vit_small_ca_forget0; do
  ...
  out="results/analysis/recovery_radius_cifar100_vit_small_forget0_seed_42_test_with_retain_adam_margin_${tag}.json"
  if [ ! -f "$out" ]; then
    echo "=== AUDIT ${suite} ==="
