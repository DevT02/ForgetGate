#!/usr/bin/env bash
# Second-seed (123) cross-architecture replication on ResNet18/CIFAR-100: train
# each LoRA unlearning method on the seed-123 base, audit, so the cross-arch
# fragility-confound + super-elasticity result can be pooled across TWO seeds.
...
for suite in unlearn_salun_resnet18_ca_forget0 unlearn_scrub_resnet18_ca_forget0 unlearn_ceascent_resnet18_ca_forget0 unlearn_uniformkl_resnet18_ca_forget0 unlearn_featurescrub_resnet18_ca_forget0 unlearn_smoothedmargin_resnet18_ca_forget0; do
  ...
  if [ ! -f "${dir}/adapter_model.safetensors" ]; then
    ...
    conda run -n forgetgate python scripts/2_train_unlearning_lora.py --config "$CFG" --suite "$suite" --seed "$SEED"
  ...
  out="results/analysis/recovery_radius_cifar100_resnet18_forget0_seed_${SEED}_test_with_retain_adam_margin_${tag}.json"
  if [ ! -f "$out" ]; then
    echo "=== AUDIT ${suite} (seed ${SEED}) ==="
    conda run -n forgetgate python scripts/audits/17_recovery_radius_audit.py --config "$CFG" --model-suites "$suite" --attack-loss adam_margin --seed "$SEED" --max-forget-samples 48 --max-retain-samples 48
  else
    echo "=== SKIP AUDIT ${suite} (present) ==="
  fi
done
echo "=== CROSSARCH SEED ${SEED} COMPLETE ==="
