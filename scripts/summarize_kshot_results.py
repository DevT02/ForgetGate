...
def main():
    ...
    lines.append("")
    if any(scrub_sources[k] for k in kshots_default):
        lines.append(
            "Coverage note: SCRUB follow-up rows are built from whatever matching logs exist in this worktree."
        )
        lines.append(
            "For each seed, the summarizer prefers `vpt_resurrect_scrub_forget0_{k}shot` and falls back to"
        )
        lines.append(
            "`vpt_resurrect_scrub_forget0_10shot_kshot{k}` when that is the only tracked variant present."
        )
        override_seeds = sorted(
            seed for seed, suite in scrub_sources[25].items() if suite.endswith("_10shot_kshot25")
        )
        if override_seeds:
            lines.append(
                f"For k=25 in this worktree, override logs are used for seeds: {', '.join(map(str, override_seeds))}."
            )
    else:
        lines.append("Coverage note: no SCRUB follow-up logs were found in this worktree.")
