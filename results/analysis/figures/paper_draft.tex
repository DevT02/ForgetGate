...
\end{equation}
where $F$ is the forget set, $R$ is the retain set, $m_t$ is the forgotten-class target margin, $\delta^*(x)$ is an inner-loop or amortized attack, and $\gamma$ is a safety margin. The implemented feature-subspace stage-2 defense can be viewed as a computationally cheaper approximation to this idea: defend against locally recoverable targeted behavior, not just against clean forget accuracy.
...
\section{Conclusion}
Machine unlearning cannot be evaluated reliably with clean forget accuracy alone. Across this benchmark, the dominant leakage channel is strong per-example conditional recovery, but local patch, frame, and multi-patch attacks expose additional method-specific surfaces that do not reduce to a single universal vulnerability. The defense picture is similarly non-universal: feature-subspace stage-2 helps several rows, patch-aware training helps ORBIT and CE-U locally, and transfer across delivery surfaces remains unsolved.
