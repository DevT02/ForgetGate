...
\begin{abstract}
Machine unlearning methods are supposed to make models forget specific training data, but in practice they often retain that information in subtle ways. I study how recoverable this hidden knowledge is across eight unlearning methods---SalUn, SCRUB, RURK, CE-U, SGA, BalDRO, ORBIT, and robust-base SalUn---attacking them four ways: universal perturbations, targeted per-example conditional recovery, local patch-based attacks (single patch, border frame, multi-patch), and amortized short-budget attacks on a controlled CIFAR-10 benchmark. The main finding is that the threat is not one-size-fits-all: different methods fail under different attacks, and a defense that works against one type often does not transfer to another. Concretely, patch-aware stage-2 defenses reduce 32$\times$32 conditional patch recovery on ORBIT (78.1\%$\to$31.2\%) and CE-U (68.8\%$\to$43.8\%) but fail against border-frame and multi-patch delivery, establishing local-delivery transfer as an open problem. I also show that a post-hoc feature-subspace defense partially reduces recoverability on BalDRO, SalUn, and ORBIT without collapsing clean accuracy. The results suggest that evaluating unlearning on clean forgetting accuracy alone is insufficient---the residual knowledge is there, it is just hard to see. Code and checkpoints are available at \url{https://github.com/DevT02/ForgetGate}.
\end{abstract}
